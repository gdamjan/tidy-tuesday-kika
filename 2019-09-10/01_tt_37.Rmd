...
![tidiverse workflow](01_tt_37_files/Screenshot_20190909_195556.png)
...
- Податоци: https://github.com/rfordatascience/tidytuesday/tree/master/data/2019/2019-09-10
...
tx_injuries_na_fixed <- tx_injuries %>% mutate_all(.funs = fix_na, na_strings = possible_na)
```
...
tx_injuries <- tx_injuries %>% 
    mutate(injury_date = case_when(
      str_length(injury_date) == 5 ~ as_date(as.numeric(injury_date), origin='1899-12-30'),
...
![glimpse](01_tt_37_files/Screenshot_20190910_113458.png)
...
ggplot(data = drop_na(tx_injuries, gender)) + 
  geom_histogram(mapping = aes(x = age, fill = gender), na.rm = TRUE, color="white") +
  labs(x="Години", y="Број") +
  labs(title="Број на повреди по години по пол", fill = "Пол") +
  theme_minimal()
#овде сакам да ги нема NA ? 
```
...
```{r, echo=TRUE, eval=FALSE}
ggplot(data = drop_na(tx_injuries, gender)) + 
  geom_point(mapping = aes(x = injury_date, y = age, color=gender)) +
  labs(x="Година", y="Возраст") +
  labs(title="Повреди по години по возраст") +
  theme_minimal()
# боја по пол? дали можеме некако да зумираме дека личи како патерн по средина можда летни месеци се тоа?
```
...
tx_injuries %>% select(age, gender, injury_date) %>% 
  ...
  drop_na() %>% 
  ggplot(data=.) + 
    geom_density_ridges(mapping = aes(x=Day_of_year, y=Year, fill=gender), alpha=.5) +
